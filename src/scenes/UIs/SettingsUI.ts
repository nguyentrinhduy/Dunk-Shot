...
export class SettingsUI extends UI {
    ...
    public create(): void {
        ...
        this.soundsToggle = new ToggleButton(this.scene, 550, 200, this.settingsManager.getSounds(), () => {
            this.settingsManager.toggleSounds()
        })
        this.vibrationToggle = new ToggleButton(this.scene, 550, 400, this.settingsManager.getVibration(), () => {
            this.settingsManager.toggleVibration()
        })
        this.nightModeToggle = new ToggleButton(this.scene, 550, 600, this.settingsManager.getNightMode(), () => {
            this.settingsManager.toggleNightMode()
        })

        this.soundsText = this.scene.add.text(100, 190, 'SOUNDS', {fontSize: '40px', color: 'black', fontStyle: 'bold'})
        this.vibrationText = this.scene.add.text(100, 390, 'VIBRATION', {fontSize: '40px', color: 'black', fontStyle: 'bold'})
        this.nightModeText = this.scene.add.text(100, 590, 'NIGHT MODE', {fontSize: '40px', color: 'black', fontStyle: 'bold'})
